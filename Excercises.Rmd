...
##2
x[x %% 2==1]
#For ||, && and isTRUE, only  length-one logical vector.
x[(x%%3==0)&&!(x%%5==0)]
...
```{r}
library(haven)
Therapy <- read_sav("/Users/mirko/Dropbox/UNI/Psychologie/Datenanalyse mit R/Daten/Therapy.sav")
...
library(dplyr)
library(readr)
rt_wide <- read_csv("/Users/mirko/Dropbox/UNI/Psychologie/Datenanalyse mit R/Daten/RTdata.csv")
...
westost <- read_sav("/Users/mirko/Dropbox/UNI/Psychologie/Datenanalyse mit R/Daten/Beispieldatensatz.sav")
westost
...
# von wide zu long
unterstuetzung <- unterstuetzung %>%
    gather(key = freunde_eltern,
           value = unterstuetzung, -ID, -westost)
# freunde_eltern variable zu factor konvertieren
unterstuetzung <- unterstuetzung %>%
...
zufriedenheit_wide <- westost %>%
    select(ID, starts_with("leben_"), -ends_with("gesamt"))
zufriedenheit_wide

zufriedenheit_long <- zufriedenheit_wide %>% 
    gather(key = lebensbereich, value = zufriedenheit, -ID)
...
library(tidyr)
library(dplyr)
titanic_data<-read_csv("/Users/mirko/Dropbox/UNI/Psychologie/Datenanalyse mit R/Daten/titanic.csv")
